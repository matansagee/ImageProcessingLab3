package il.ac.tau.adviplab.mylibrary;
...
public class MyImageProc {
    ...
    public static final int BETA_MAX = 1;
...
//Applies the Sobel filter to image
    ...
    public static void detecetAndReplaceChessboard(Mat sourceImage, Mat
            replacementImage){
// sourceImage - a sourceImage with a chessboard pattern
//replacementImage - an image to replace the chessboard

        int bwThresold = 100;
        Mat bwImage = new Mat();
        MyImageProc.im2BW(sourceImage, bwImage, bwThresold,
                Imgproc.THRESH_BINARY_INV);


    }

    public static void im2BW(Mat image, Mat bwImage, double threshold, int
            type) {
     // type can be either Imgproc.THRESH_BINARY_INV or Imgproc.THRESH_BINARY
     //todo: Add your implementation here

        Imgproc.threshold(image,bwImage,threshold,type,0);
